---
icon: fas fa-info-circle
order: 4
---

## Hi, I'm Pramesh 👋

I'm an Electrical Engineering student at UC San Diego (Jacobs School of Engineering) with specializations in **Machine Learning & Controls** and **Computer System Design**, plus a minor in Business. But that's just the technical side of things.

### My Journey

Born in Indonesia, raised across Brazil and Singapore for 15 years, now based in San Diego—my background is as international as it gets. Growing up across continents taught me to adapt quickly, see problems from multiple angles, and explain where I'm from in under five seconds (still perfecting that elevator pitch).

Before engineering, I spent two years in the **Singapore Armed Forces** as a National Serviceman, training in the intelligence branch. Leading a 30-member platoon in high-pressure operations sharpened my strategic thinking, problem-solving under constraints, and ability to function on minimal sleep—skills that translate surprisingly well to debugging code at 3 AM.

### The Athletic Chapter

Cricket was my first love. I vice-captained Singapore's **U16 and U19 national teams**, competed internationally, and was part of the first youth team to defeat Hong Kong. Those years taught me discipline, teamwork, and how to perform under pressure—whether facing a fast bowler or a tight project deadline.

### What I'm Building

My technical interests lie at the intersection of **AI, robotics, and systems design**:

**Recent Projects:**
- **OpsPilot** - Built an AI-powered reconciliation system using FastAPI and Claude API to automate trade validation and anomaly detection
- **Sim-to-Real Robot Navigation** - Led a team developing RL models in Unreal Engine for autonomous PiCar navigation, collaborating with AMD engineers
- **AI Marketing Optimization** - Created a GPT-based content generation pipeline that improved CTA clarity by 67% for small businesses

**Professional Experience:**
- **Monitor Deloitte** (Strategy Consultant Intern) - Market entry strategy for pharmaceutical clients in Indonesia
- **Ernst & Young** (Technology Consultant Intern) - Cybersecurity frameworks for Port Singapore Authority

### Beyond Engineering

I'm deeply curious about how different fields connect. I've minored in Finance (markets are just puzzles with money), previously studied Theatre (why not?), and love finding unexpected parallels—like how cricket strategy relates to business decision-making, or how AI intersects with creativity.

When I'm not coding or studying, you'll find me:
- Running marathons (completed one, still recovering)
- Experimenting in the kitchen (with mixed results)
- Reading about nuclear fusion and astrophysics
- Drinking coffee and trying new wines!
- Anything outdoors!

### Current Focus

**Academically:** Machine Learning, Control Systems, Computer Architecture  
**Professionally:** Building AI systems that bridge simulation and reality  
**Leadership:** President of UCSD Cricket Club, VP of Sigma Alpha Mu Fraternity  
**GPA:** 3.72

### Technical Skills

**Languages:** Python, C/C++, Java, MATLAB, HTML  
**Frameworks & Tools:** FastAPI, SQLAlchemy, Stable-Baselines3, Unreal Engine, Tableau  
**Areas:** Machine Learning, Reinforcement Learning, Computer Architecture, Cybersecurity, Control Systems

### Let's Connect

I'm always interested in discussing AI/ML projects, robotics, strategy consulting, or just exchanging ideas about how different fields intersect.

📧 **Email:** [prameshsinghavi02@gmail.com](mailto:prameshsinghavi02@gmail.com)  
💼 **LinkedIn:** [Pramesh Singhavi](https://www.linkedin.com/in/pramesh-singhavi-81bab6286/)  
💻 **GitHub:** [@prams2104](https://github.com/prams2104)  
📝 **Blog:** [The Uncharted Angles](https://www.theunchartedangles.com/)  
📄 **Resume:** [Download PDF](/assets/resume/Pramesh_Resume.pdf)

---

*"Welcome to the rabbit hole."*